...
class MUNkeyLinkElementHandler(InlineEntityElementHandler):
    ...
    def get_attribute_data(self, attrs):
        return {"name": attrs["linktype"], "linktype": "munkey"}
...
def munkey_link_decorator(props):
    ...
    return DOM.create_element(
        "a", {"linktype": "munkey", "name": props["name"]}, props["children"]
    )
...
def register_munkey_link(features):
    ...
    features.register_editor_plugin(
        "draftail",
        feature_name,
        draftail_features.EntityFeature(control, js=["navigation/js/MUNkeyLINK.js"]),
    )
